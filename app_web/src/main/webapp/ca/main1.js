...
***********************************************/	
function WebSign_AddSeal(sealName,lPosX,lPosY, sealPostion,signData){
	try{
		//是否已经盖章
		...
		while(strObjectName  != ""){
			if(sealName == strObjectName){
				...
				$.messager.alert("系统提示","当前页面已经加盖过印章！","warning");
				return false;
			}
		...
		//网络版服务器路径
		//document.all.DWebSignSeal.HttpAddress = "http://192.168.0.199:8089/inc/seal_interface/";
		...
		//这样就可以很好的固定印章的位置
		document.all.DWebSignSeal.SetPosition(lPosX,lPosY,sealPostion);
		//调用盖章的接口
		var sealid = document.all.DWebSignSeal.AddSeal("", sealName);
		return true;
	}catch(e) {
		$.messager.alert("系统提示","控件没有安装，请刷新本页面，控件会自动下载。\r\n或者下载安装程序安装。" +e);
		return false;
	}
}	
/*******************全屏幕手写****************************
...
function HandWritePop_onclick(){
	try{ 
		...
		//调用签名的接口
		if("" == document.all.DWebSignSeal.HandWritePop(0,255,50, 400,300,"")){
			 alert("全屏幕签名失败");
			 return false;
		}
	}catch(e) {
...
function Enc_onclick(tex_name) {		
	try{
		...
		如果绑定字符串数据,需要做如下调用*/
		document.all.DWebSignSeal.SetSignData("+DATA:"+tex_name);		
	}catch(e) {
		alert("控件没有安装，请刷新本页面，控件会自动下载。\r\n或者下载安装程序安装。" +e);
	}
}
