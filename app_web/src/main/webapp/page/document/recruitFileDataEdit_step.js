$.extend($.fn.validatebox.defaults.rules, {
	...
	fullScore : {
		validator : function(value, param) {
			...
			if(uniqueMethodId =="JT_ZHPF_METHOD"){
				return true;
			}
...
$(function() {
	...
	$("#reviewMode").change(function(){
		...
		if(this.value=="PASS"){
			$("#scoreGroup").hide();
			$("#passDetailReview").show();
		}else{
			$("#scoreGroup").show();
			$("#passDetailReview").hide();
		}
	}).change();*/
	/***************************************************************************
	 * 货币金额大写计算
	...
	function serializeData() {
		...
		if($("#reviewMode").val()=="SCORE"){
			var index = $("#zbwjForm").find("#scoreGroup").children("tbody.editor").children("tr").length ;
	...
	function saveRecruitFileData(isComplete) {
		...
		$.ajax({
			...
			success : function(data) {
				$.jGrowl(data.msg);
				if(data.success){
					window.location.href="/recruitFileData/goToRecruitFileDataByStep?id=" + data.data + "&nextStep=2_1";
					//FIXME 还原
					//history.go(-1);
				}			
			},
	...
	function saveRecruitFileDataByStep(isComplete) {
		if($("#reviewMode").val() == "SCORE"){
		...
		$.ajax({
			url : ctx + "/recruitFileData/saveOrUpdateRecruitFileDataByStep",
			type : "post",
			dataType : "json",
			cache : false,
			beforeSend : function() {
				$("#save_next_step,#step_save,#step_complete").prop("disabled", true);
				$.messager.progress({
					title : "系统提示",
					msg : "正在提交数据"
				});
			...
			success : function(data) {
				...
				if(data.success){
					if(data.data){
						window.location.href="/recruitFileData/goToRecruitFileDataByStep?id=" + $("#id").val() + "&nextStep=" + data.data;
					}else{
			...
			complete : function() {
				$("#save_next_step,#step_save,#step_complete").prop("disabled", false);
				$.messager.progress("close");
			}
		});
	...
	*//***************************************************************************
	...
	$("#save_next_step").click(function(){
		saveRecruitFileDataByStep(false);
	});
	
	$("#step_complete").click(function(){
		saveRecruitFileDataByStep(true);
	});
