	...
	$("#attachmentUpload").datagrid({
		...
		columns:[[
		  ...
		  {title:"操作",field:"opt",width:120,align:'center',formatter:function(value,rowData,rowIndex){
			  var flag=notifyForm.view==undefined?0:notifyForm.view.value;
			  return (rowData["path"]?(flag==1?"<a target=\"_blank\" href=\""+ctx+"/attachment/download/"+rowData["path"]+"\">下载</a>":"<a id=\""+rowData["fjid00"]+"\" class=\"del\">删除</a>&nbsp;<a target=\"_blank\" href=\""+ctx+"/attachment/download/"+rowData["path"]+"\">下载</a>"):"");
		  }}
		]]
	});
	/*$("#attachment").datagrid({
	...
	var uploader = new plupload.Uploader({
		...
		init : {
			...
			FileUploaded : function(up, file, result) {
				...
				$("#notifyForm").append($("<input/>").attr("type","hidden").addClass(file.id).val(data.data.fjid00));
				up.removeFile(file);
				//loadDefaultNoticeContent();
