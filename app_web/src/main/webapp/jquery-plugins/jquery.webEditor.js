...
//		});
		 ...
		 */
		rangy.init();
		var loadIframeCount=0;
		$(".viewport iframe").load(function(){
			loadIframeCount++;
			//TODO
			if(loadIframeCount==$(".viewport").length&&false){
				var iframe=this;
				window.setTimeout(function(){
					var data=$(".viewport iframe").contents().find("body .jckj-div").filter(function(index){
		         ...
		         if (rangy.supported && classApplierModule && classApplierModule.supported) {
						window.setTimeout(function(){
			                ...
			                var options = {
			                    ...
			                    wholeWordsOnly: false,//全词匹配
			                    withinRange: false,//支持正则
			                    direction: "forward" // This is redundant because "forward" is the default
			                };
			                range.selectNodeContents(document.body);
			                searchResultApplier.undoToRange(range);
			                if (value !== "") {
			                    while (range.findText(value, options)) {
	...
	function createToolBar($parent){
		...
		this.toolbar[1]={text:"文件",iconCls:"myicon-page_word",plain:true,menuOptions:{root:[{text:"打开",iconCls:"myicon-folder_page",disabled:!options.openable},{separator:true},{text:"保存",iconCls:"myicon-disk",disabled:true},{text:"另存为",iconCls:"myicon-drive_disk",disabled:true},{separator:true},{text:"导出",iconCls:"myicon-page_white_word"},{separator:true},{text:"退出"}]}};
		...
		this.toolbar[7]={id:"create_send_file",text:"生成投标文件",iconCls:"myicon-table_edit",plain:true,onClick:function(){
			...
			$("li[name='send_file']").children("pre").remove();
	...
	function createCurrentCssRule2(options){
		//TODO
		var rules = this.styleSheets[options.styleSheetIndex].cssRules?this.styleSheets[options.styleSheetIndex].cssRules:this.styleSheets[options.styleSheetIndex].rules;
		for(var key in $.fn.webEditor.methods.cssRules){
			...
			if(this.styleSheets[options.styleSheetIndex].addRule){
				this.styleSheets[options.styleSheetIndex].addRule(selector,ruleText,ruleIndex);  
			}else{
				this.styleSheets[options.styleSheetIndex].insertRule(selector+"{"+ruleText+"}",ruleIndex);
			}
		}
	...
	function createVariableInput(name){
		return insertObject.call(this,$("<span/>").addClass("need_input u").attr("name",name).html("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"));
	}
	function createVariableTool(){
		var options=this;
		return [
        	{text:"招标项目名称",handler:function(){
        		return createVariableInput.call(options,createVariableInput("projectInstance.zbxmmc"));        	
        	}},
        	{text:"招标项目编号",handler:function(){
        		return createVariableInput.call(options,createVariableInput("projectInstance.organProjectCode"));            	
        	}},
        	{text:"招标内容与范围",handler:function(){
        		return createVariableInput.call(options,createVariableInput("projectInstance.nryfw0"));           	
        	}},
        	{text:"资金来源",handler:function(){
        		return createVariableInput.call(options,createVariableInput("project.funding"));            	
        	}},
        	{text:"资格审查方式",handler:function(){
        		return createVariableInput.call(options,createVariableInput("dictionaryService.getDictionary('ZGSHFS',projectInstance.zgshfs)"));            
        	}},
        	{text:"招标方式",handler:function(){
        		return createVariableInput.call(options,createVariableInput("dictionaryService.getDictionary('ZBFS00',projectInstance.zbfs00)"));    
        	}},
        	{text:"招标组织形式",handler:function(){
        		return createVariableInput.call(options,createVariableInput("dictionaryService.getDictionary('ZBZZXS',projectInstance.zbzzxs)"));    
        	}},
        	{text:"交货期",handler:function(){
        		return createVariableInput.call(options,createVariableInput("projectItem.deliveryTimeLimit"));    
        	}},
        	{text:"交货地点",handler:function(){
        		return createVariableInput.call(options,createVariableInput("tenderNoticeData.deliveryPlace"));            	
        	}},
        	{text:"投标报价",handler:function(){
        		return createVariableInput.call(options,createVariableInput("bidPrice"));            	
        	}},
        	{text:"投标报价(大写)",handler:function(){
        		return createVariableInput.call(options,createVariableInput("bidPriceChinese"));            	
        	}},
        	{separator:true},
        	{text:"招标人",handler:function(){
        		return createVariableInput.call(options,createVariableInput("organT.ztmc00"));    
        	}},
        	{text:"联系人(招标人)",handler:function(){
        		return createVariableInput.call(options,createVariableInput("organT.contacts"));    
        	}},
        	{text:"联系方式(招标人)",handler:function(){
        		return createVariableInput.call(options,createVariableInput("organT.phones"));    
        	}},
        	{separator:true},
        	{text:"代理机构",handler:function(){
        		return createVariableInput.call(options,createVariableInput("organA.ztmc00"));    
        	}},
        	{text:"联系人(代理机构)",handler:function(){
        		return createVariableInput.call(options,createVariableInput("organA.contacts"));    
        	}},
        	{text:"联系方式(代理机构)",handler:function(){
        		return createVariableInput.call(options,createVariableInput("organA.phones"));    
        	}},
        	{separator:true},
        	{text:"投标人",handler:function(){
        		return createVariableInput.call(options,createVariableInput("organB.ztmc00"));    
        	}},
        	{text:"法人(投标人)",handler:function(){
        		//TODO
        		return createVariableInput.call(options,createVariableInput("fanren"));    
        	}}
        ];
	};
	/** 创建插入表格工具* */
		...
		if(!options.scale.block){
			...
			window.setTimeout(function() {
				...
				if(options.scale.newValue!=options.scale.oldValue){
					for(var i=0,l=options.components.viewports.length;i<l;i++){
						options.components.viewports[i].find("iframe").contents().find("body").css("zoom",options.scale.newValue);
						//var zoomWith=options.components.viewports[i].find("iframe").contents().find("body").width()*options.scale.newValue;
						//var realWith=options.components.viewports[i].find("iframe").contents().find("body").width();
					}
	...
	function getData($li) {
		...
		data["parent"] = $parentLi.attr("id") ? $parentLi.attr("id") : "";
	...
	function param(){
		...
		$("li[changed='create']").filter(function() {
			return $(this).parent("ul").parent("li").attr("id");
		}).each(function(i) {
		...
		$(".tooltip[changed='create']").filter(function() {
			...
			paramArray.push({name:"createComments[" + i + "].itemId",value:$(this).data("li").attr("id")});
		...
		$(".tooltip[changed='update']").each(function(i) {
			paramArray.push({name:"updateComments[" + i + "].itemId",value:$(this).data("li").attr("id")});
		...
		$(".tooltip[changed='delete']").each(function(i) {
			paramArray.push({name:"deleteComments[" + i + "].itemId",value:$(this).data("li").attr("id")});
	...
	/** 文档初始化* */
	function initDocument($viewport){
		var options=this;
		...
		$viewport.on("focus",".my_word_document>.my_word_center>.my_word_body pre",function(e){
			...
			if($li.attr("id")&&!$pre.parent("li").data("content")){
		...
		**************************************************/
		$viewport.on("blur",".my_word_document>.my_word_center>.my_word_body pre",$viewport[0],function(e){
			...
			$(this).prepend($(this).data("index")).removeData("index");
		});
		 ...
		 * 光标上下移
		 *************/
		$viewport.on("keydown",".my_word_document>.my_word_center>.my_word_body pre:focus:not(:has(table))","up",function(e){
		...
		$viewport.on("keyup",".my_word_document>.my_word_center>.my_word_body pre:focus:not(:has(table))","up",function(e){
			var selection=rangy.getSelection();
			var currentRange=getFirstRange();
			if(!currentRange){
				return false;
			}
			var currentFocusNode=selection.focusNode;
			var currentStartOffset=currentRange.startOffset;
			if(currentFocusNode==focusNode&&currentStartOffset==startOffset){
				var prev=$(this).parent().prev();
				if(prev.length==1){
					prev.children("pre").focus();
				}
			}
			focusNode=currentFocusNode;
			startOffset=currentStartOffset;
		});
		$viewport.on("keydown",".my_word_document>.my_word_center>.my_word_body pre:focus:not(:has(table))","down",function(e){
			var selection=rangy.getSelection();
			focusNode=selection.focusNode;
			var range=getFirstRange();
			if(!range){
		...
		$viewport.on("keyup",".my_word_document>.my_word_center>.my_word_body pre:focus:not(:has(table))","down",function(e){
			var selection=rangy.getSelection();
			var currentRange=getFirstRange();
			if(!currentRange){
				return false;
		...
		$viewport.on('keydown',".my_word_document>.my_word_center>.my_word_body pre:focus:not(:has(table))", 'backspace',function (e){
        	...
        	if($(this).parent().prev().children("pre").find("table").length>0){
        		return false;
        	}
        	$(this).children("span.index").remove();
			var thisHtml=$(this).html();
			var lastChild=$(this).parent().prev().children("pre")[0].lastChild;
			$(this).blur().parent("li").prev().children("pre").focus().append(thisHtml);
			...
			deleteData.call(options,$(this).parent());
			return false;
		});
		/**************************************************
		 * 回车换行处理
		...
		$viewport.on('keydown',".my_word_document>.my_word_center>.my_word_body pre:focus:not(:has(table))", 'return',function (e){
			...
			if($(startTextNode).is("pre")){
				range=rangy.createRange();
			}
			if(startTextNode.data){
				range.moveStart('character',0); 
				range.moveEnd('character',startTextNode.data.length-range.startOffset);
				range.select();
				$insertpre.append(range.extractContents());
			}
			var childNodes=[];
			while(true){
				startTextNode=startTextNode.nextSibling;
				if(startTextNode==null){
					break;
				}
				childNodes.push(startTextNode);
			}
			for(var i=0,l=childNodes.length;i<l;i++){
				var childNode=childNodes[i];
				$insertpre.append(childNode);	
			}
			$pre.blur();
			$insertpre.focus();
			if($li.attr("isTitle")=="true"){
	...
	function changeNoType($li,noType){
		...
		var data = eval("(" + $li.attr("data") + ")");
		if ($li.attr("id") && !$li.attr("changed") && data.noType != noType) {
			$li.attr("changed", "update");
		}
	...
	$.fn.webEditor.methods={
		...
		setContent:function(content){
			...
			$(".viewport-panel").each(function(i){
				$(this).panel({"content":content[i],onOpen:function(){
					...
					// 更新导航
					if(options.components.viewports.length==content.length){
						if(options.commentDisplay){
							options.toolbar[5].menuOptions.root[5].menuItem.click();
						}
						refreshNavigationtree.call(options);
		...
		refreshDocument:function(href){
			...
			$(".viewport-panel").each(function(i){
				$(this).panel({"content":"<iframe style='width:100%;height:100%;border:none;' src='"+href[i]+"'></iframe>",onOpen:function(){
					//添加viewport
					options.components.viewports.push($(this));
					//初始化文档
					initDocument.call(options,$(this));
