	...
	a.util = {
		dataType : {
			...
			number:function(gets,obj,curform,regxp){
				if(gets!=""){
					if(!/^-?([0-9]|([1-9][0-9]*)|([0-9]\.[0-9]+)|([1-9][0-9]*\.[0-9]+))$/.test(gets)){
						return false;
					}
					var value=parseFloat(gets);
					if($(obj).attr("limit")){
						var limit=$(obj).attr("limit");
						var data=limit.substring(1,limit.length-1).split(",");
						if(!((limit.charAt(0)=="["&&value>=parseFloat(data[0]))||(limit.charAt(0)=="("&&(data[0]==""||value>parseFloat(data[0]))))&&((limit.charAt(limit.length-1)=="]"&&value<=parseFloat(data[1]))||(limit.charAt(limit.length-1)==")"&&(data[1]==""||value<parseFloat(data[1]))))){
							return false;
						}
					}
					if($(obj).attr("minFractionDigits")){
						var minFractionDigits=parseInt($(obj).attr("minFractionDigits"));
						var weight=1;
		...
		enhance : function(l, m, n, k) {
			...
			o.find("input[recheck]").each(function() {
				...
				this.validform_inited = "inited";
				var q = d(this);
				var p = o.find(d(this).attr("recheck"));
