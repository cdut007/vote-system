...
<body>
	...
	<div class="block">
		...
		<div class="blank10"></div>
		<div class="tableTitle2" style="cursor:pointer;">项目信息（单击隐藏）</div>
		...
		<table class='mytableStyle'>
			...
			<c:forEach items="${projectInstance.projectItemList}" var="item" varStatus="status">
				<tr>
					<td>
						<input type='checkbox' name='item_ids' onclick="check()" value="${item.id}" <c:forEach items="${tender.tenderItem}" var="tenderItem">
			...
			<c:if test="${empty projectInstance.projectItemList}">
				<tr>
			...
			<div id="tender_application" style="width:650px;border:1px solid #ccc;line-height:30px;padding:70px;margin:10px 100px">
				<c:if test="${tender.content==null}">
					<h1 style="text-align:center;" align="center">
						<br /> <b><span style="font-family:宋体;font-size:18pt;">投标报名申请表<span></span></span></b> <br />
					</h1>
					<br />
					<div style="font-family:FangSong_GB2312;font-size:14px;">
						...
						<p style="text-indent:22pt;">
							...
							</u><span>的招标公告，我单位拟参与该工程</span><u><span id="projectItemName"> <c:forEach items="${tender.tenderItem}" var="tenderItem">
		...
		<form:form id="tenderApplicationForm" method="post" commandName="tender">
			<div style="text-align:right;position:fixed;left:0px;right:0px;bottom:0px;padding-top:10px;padding-bottom:10px;padding-right:30px;background-color:#efefef;z-index:999;border: #ccc 1px solid;">
				<input type='hidden' name="taskId" value="${task.id}" /> <input type='hidden' name="id" value="${tender.id}" /> <input type='hidden' name="tbxmzj" value="${projectInstance.id}" /> <input type='hidden' name="bdzjjh" value="${itemIDs}" /> <input type='hidden' name="bmshr0" value="${projectInstance.dlrzj0}" />
				<form:textarea id="content" path="content" cssClass="none" />
				<form:hidden path="sealSignData" id="sealSignData" />
				<input type='hidden' name="cancelReason" /> <input type="button" class='red button' id="saveBtn" value="保存" /> <input type="button" class="red button" id='sign' value="签章" /> <input type="button" id='subBtn' class='red button' value="提交" /> <input type="button" class='red button' id="cancelBtn" value="撤销申请" />
			</div>
		</form:form>
		<div class="blank10"></div>
		<div class="blank10"></div>
