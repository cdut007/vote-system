		...
		$(function() {
			...
			$("#pass").click(function() {
				...
				if(r==1){
	                /* var m = new Map();
	                m.set("taskId","${taskId}");
	                m.set("DocumentID","${recruitFileAddendum.id}");
	                m.set("id","${recruitFileAddendum.id}");
	                m.set("pass","true");
	                var returnValue = SaveDoc(m,"${pageScope.basePath}/recruitFile/recruitFileAddendumReview"); */
					var paramsArray = {"taskId":"${taskId}", "DocumentID":"${recruitFileAddendum.id}", "id":"${recruitFileAddendum.id}","pass":"true"};
	                var returnValue = SaveDocArray(paramsArray,"${pageScope.basePath}/recruitFile/recruitFileAddendumReview");
	                if ("OK" == returnValue) {
	                    ShowMessage("文件上传成功");
