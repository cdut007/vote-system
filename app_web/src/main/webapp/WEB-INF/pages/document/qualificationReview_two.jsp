...
<%@taglib prefix="form" uri="http://www.springframework.org/tags/form"%>
	<table id="passDetailReview" class="mytableStyle"> 
		...
		<tbody class="editor">
			<c:forEach items="${evaluationMethodJson.detailedReviewTableJsonTwo}"
						...
						<table class="mytableStyle">
							<thead>
								<tr>
									...
									<th style="width: 200px">评审因素与评审标准</th>
								</tr>
							</thead>
							<tbody class="editor">
								<c:forEach items="${detailed.psnr}" var="psnr" varStatus="s">
									<tr>
										<th>${psnr.id}</th>
										<td><textarea name="evaluationFactor" readonly="readonly">${psnr.psbz}</textarea>
										</td>
									</tr>
								</c:forEach>
							</tbody>
