...
<table id="scoreGroup" class="mytableStyle">
	...
	<tr>
		...
		<c:forEach items="${evaluationMethodJson.detailedReviewTableJsonTwo}" var="xiangPingXiangMu">
				...
				<tbody  class="other" >
					<tr>
						...
						<td><input type="hidden" name="flag" value="price" /><input
							type="hidden" name="groupName" value="${xiangPingXiangMu.fzgc }" />${xiangPingXiangMu.fzgc }</td>
						<td><input type="hidden" name="needUnify" value="${xiangPingXiangMu.sfhy=='no'?0:1}" />${xiangPingXiangMu.sfhy=='no'?'否':'是'}</td>
