...
<script type="text/javascript">
	...
	function changeItem(){
		...
		$.post("${ctx}/bidNotify/findNoPassOrganByProjectItem",{projectItemId:projectItemId},function(data){//alert(data);
			var jsonArray=eval("("+data+")");
			if(jsonArray.length>0){
				organIds=jsonArray[0];
				organNames=jsonArray[1];//alert(organNames);
				bidOrgan=document.getElementById("bidOrgan");
				bidOrgan.innerHTML=organNames;
			}else{
				organIds="";
				organNames="";
				bidOrgan=document.getElementById("bidOrgan");
	...
	function doSave(){//alert("doSave");
		...
		var requestParam={
				...
				organName:organNames/*,
				days:days,
				linkman:linkman*/
		};//alert(requestParam.organName);
	...
	function loadContent(){//alert("loadContent");
		...
		$.ajax({
			url:ctx+"/bidNotify/loadNoPassNotifyContent",//loadNoPassNotifyContent
			type:"get",
			data:requestParam,
			dataType:"html",
			...
			<form:form id="notifyForm" method="post" action='${pageContext.request.contextPath}/notify/save' modelAttribute="notify" onsubmit="return $(this).form('validate');">
				...
				<table width="100%" style="table-layout:fixed;">
					<tr>
						...
						<td>
						<%--<form:input id="days" cssStyle="width:30%" path="days" cssClass="inputxt easyui-validatebox" data-options="required:true,validType:'length[0,16]'" missingMessage="请输入联系人" onblur="loadContent()" value="${notify.days}" onkeyup="value=value.replace(/[^\d]/g,'')"/></td> --%>
						<td class="need" style="width:10px;">&nbsp;</td>
						<td >&nbsp; <%-- <form:input id="linkman" cssStyle="width:30%" path="linkman" cssClass="inputxt easyui-validatebox" data-options="required:true,validType:'length[0,16]'" missingMessage="请输入联系人" onblur="loadContent()" value="${notify.linkman}"/>--%></td>
						 </tr>
					<tr>
						<td class="need"></td>
						<td>通知内容</td>
