...
<script type="text/javascript">
	...
	$(function() {
	    $("#sign").click(function(){
	        SearchText("递交于我公司",0,0);
	        if(AutoSeal(0,1,"递交于我公司")=="-200"){
		...
		$("#sub").click(function() {
			...
			if(r==1){
                //var m = new Map();
                //m.set("clarifyFile","${clarifyFile}");
                //m.set("id","${clarifyFile.id}");
                //var returnValue = SaveDoc(m,"${pageScope.basePath}/question/questionSign"); 
                
                var paramsArray = {"clarifyFile":"${clarifyFile}", "id":"${clarifyFile.id}"};
                var returnValue = SaveDocArray(paramsArray,"${pageScope.basePath}/question/questionSign");
                if ("OK" == returnValue) {
                    ShowMessage("文件上传成功");
	...
	function HWPostil1_NotifyCtrlReady() {
        ...
        OpenFile("${pageScope.basePath}/attachment/download/${clarifyFile.pdfAttachment.fjid00}");
