...
<script type="text/javascript">
	$(function() {
		var bidOpeningQuestionId = "${bidOpeningQuestion.id}";
		$("#id").val(bidOpeningQuestionId);
		...
		$("#sub").click(function() {
			...
			$("#bidOpeningQuestionAnswerForm").ajaxSubmit({
				dataType:"json",
				beforeSend:function(){
					$.messager.progress({
						title:"系统提示",
				...
				success : function(data) {
					if(data.success){
						$.messager.alert("系统提示","操作成功，请签章后提交！","info",function(){
							window.location.href=ctx+"/question/confirmBidOpeningQuestionAnswer?id="+data.data;
						});
					}
				},
				complete:function(){
...
<body>
	...
	<form id="bidOpeningQuestionAnswerForm" action="${pageContext.request.contextPath}/question/saveBidOpeningQuestionAnswer" method="post">
		<div class="block">
			...
			<table style="width:100%;">
				...
				<tr>
					<td>澄清通知：</td>
					<td>
						<div id="content" style="padding:50px;border:1px solid #ddd;"><jsp:include page="bidOpeningQuestionDefault.jsp" /></div>
					</td>
				</tr>
			</table>
				<input type="hidden" id="questionAnswer" name="questionAnswer" />
				<input type="hidden" id="id" name="id" value="${bidOpeningQuestion.id }" />
