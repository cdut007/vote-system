...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#passBtn").click(function(){
			...
			if(r==1){
                ...
                m.set("pass","true");*/var paramsArray = {"taskId":"${taskId}", "attachId":$("#attachId").val(),"pass":"true"
                		,"userId":$("#userId").val(),"itemId":$("#itemId").val(),"status":$("#status").val()};
                var returnValue = SaveDocArray(paramsArray,"${pageScope.basePath}/guardianAudit/auditTenderSign");
                if ("OK" == returnValue) {
                    ShowMessage("文件上传成功");
    ...
    function HWPostil1_NotifyCtrlReady() {
        ...
        OpenFile("${pageScope.basePath}/attachment/download/${userItem.attachId}");
	...
	<div class="block">
		...
		<div class="usBox main" style="padding: 3px;height:660px;" id="ht_content">
			...
			<div style="text-align:right;">
				<form:form id="qdhtForm" method="post" action="${pageContext.request.contextPath}/workflow/completeForm" modelAttribute="userItem">
					<input type='hidden' id="taskId" name='taskId' value="${taskId}" />
					<input type='hidden' id="attachId" name='attachId' value="${userItem.attachId}"/>
					<input type="hidden" id="userId" name="userId" value="${userItem.userId}"/>
					<input type="hidden" id="itemId" name="itemId" value="${userItem.itemId}"/>
					<input type="hidden" id="status" name="status" value="${userItem.status}"/>
