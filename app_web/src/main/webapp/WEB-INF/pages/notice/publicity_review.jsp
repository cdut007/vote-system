...
function HWPostil1_NotifyCtrlReady() {
	...
	$(function() {
		...
		$("#pass").click(function() {
			...
			if(r==1){
                var paramsArray = {"taskId":"${taskId}","DocumentID":"${candidatePublicity.id}", "id":"${candidatePublicity.id}","pass":"true"};
                var returnValue = SaveDocArrayAsPdf("test_sealed.pdf",paramsArray,"${pageScope.basePath}/candidatePublicity/candidatePublicityReviewTOrA");
                if ("OK" == returnValue) {
                    ShowMessage("文件上传成功");
	...
	<div class="block">
		<div class="usBox">
			...
			<form:form id="publicityForm" method="post" action='${pageContext.request.contextPath}/workflow/completeForm' modelAttribute="candidatePublicity">
				...
				<input type="hidden" name="DocumentID" value="${candidatePublicity.id}" />
