...
$(function() {
	...
	dg= $("#bidFile-datagird").datagrid({
		...
		rownumbers : true,
		striped : true,
		url:ctx+"/bidFile/bidfile_revise_list",
		queryParams : {
		...
		columns:[[
			...
			{field : 'organProjectItemCode',width:150,title : "标段编号"},
			{field : 'projectItemName',width:200,title : "标段名称"}, 
			{field : 'organBName',title : "投标单位",width:300,align : "center"}, 
			...
			{field : 'reviseTbbjje',title : "修正报价",width : 160, editor:{type:'numberbox',options:{precision:1}}, formatter : function(value, row, index){
			...
			/* {field : 'gq0000',title : "工期（交货期、服务周期）",width : 150} */
...
<body>
	...
	<div class="block">
		...
		<div style="color:red;">提示：修正报价列填入重新修正的价格，按回车键确认填写完成</div>
		...
		<div style="text-align:right;padding-top:10px;padding-bottom:10px;padding-right:5px;z-index:998;">
			<form id="workflowForm" action="${pageContext.request.contextPath}/workflow/completeForm" method="post">
				...
				<a class="button red" id="complete">发起修正</a>
				<a class="button red" id="skip">不需要修正</a>
			</form>
		</div>
...
$("#complete").click(function() {
	...
	$(rows).each(function(){
		...
		if(this["reviseTbbjje"]!="" && this["reviseTbbjje"]){
			
			var data = {
					id: this["id"],
					reviseTbbjje: this["reviseTbbjje"]
			}
	...
	if(revise_list.length>0){
		$.messager.confirm("系统提示","当前有" + revise_list.length + "家投标单位报价进行修正，是否提交专家签字?",function(r){
			if(r){
				$.ajax({
					url : "${pageContext.request.contextPath}/bidFile/save_bidfile_revise",
					type : "post",
					data : {
