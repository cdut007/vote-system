...
$(function () {
	...
	chatManagerService.createScriptSession({expertApplyId:$(":input[name='expertApplyId']").val()},function(){
		$.messager.alert("系统提示","连接到开标室成功!");
	...
	$("#sign-datagrid").datagrid({
		...
		columns:[[
		    ...
		    {title:"项目名称",field:'projectInstanceName',width:180,hidden:true},
		    {title:"授权人姓名",field:'userName',align:"center",width:100,formatter:function(value,row,index){
		    	if(row.userType=="专家"){
		    		return "***";
		    	}else{
		    		return value;
		    ...
		    {title:"单位名称",field:'unitName',width:100},
		    {title:"联系方式",field:'phoneNumber',width:100,formatter:function(value,row,index){
		    	if(row.userType=="专家"){
		    		return " ";
		    	}else{
		    		return value;
	...
	$("#bidFile-datagird").datagrid({
		...
		columns:[[
			...
			{field:"tenderItemId",title:"操作",width:100,formatter:function(value,rowData,rowIndex){
				return rowData["isDecrypt"]=="未解密"||rowData["isDecrypt"]=="解密失败"?"<input type=\"button\" onclick=\"decrypt1('"+value+"','"+rowData["projectItemId"]+"','"+rowData["bdmc00"]+"');\" class=\"easyui-linkbutton button red small\" value=\"解密\"/>":"";
			}}
		]]
	});
	$(".bidOpen-datagrid").each(function(){
		...
		$(this).datagrid({
			...
			columns : [ [ 
				...
				{field : 'gq0000',title : "工期/交货期/服务周期(天)",align : "center",width : 120,formatter : hidedate},  
...
<body>
	...
	<div class="block">
		...
		<div class="blank10"></div>
		<div style="float:left;width:59%;margin-right:5px;text-align:center;height:300px;">
		...
		<div style="border:1px solid #e4e4e4;width:40%;float:right;">
			<div class="video" id="CuPlayer"><b><img src="/resources/player/images/loading.gif"  /> 网页视频播放器加载中，请稍后...</b></div>
			<script type="text/javascript">
				var so = new SWFObject("/resources/player/player.swf","ply","400","300","9","#000000");
