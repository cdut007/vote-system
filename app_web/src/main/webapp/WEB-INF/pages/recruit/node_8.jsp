        ...
        $(function(){
            ...
            $("#pass").click(function(){
				if(sealNum==0){
					...
					var paramsArray = {"id":"${recruitFile.id}"};
	                var returnValue = SaveDocArray(paramsArray,"${pageScope.basePath}/recruitFile/updateRecruitFilePdf");
	...
	<div data-options="region:'south',height:40,split:false"
		...
		<form
			...
			<input type="hidden" name="taskId" value="${taskId}" /> <input
				type="hidden" name="pass" value="true" /> <input type="hidden"
				name="nopassReason" value="不通过" /> <input type="button"
				class="button red" value="签章" id="sign" /> <input type="button"
				class="button red" value="通过" id="pass" /> <input type="button"
				class="button red" value="不通过" id="unpass" />
		</form>
	</div>
</body>
</HTML>
