...
<div class="ctr content">
    ...
    <div class="address_nav">
        ...
        <div class="address_main left">
            当前位置：<a href="${util:property('frontEnd')}">首页</a> &gt; <a href="#">候选人公示</a>
        </div>
        <!-- 4.1.2通知 -->
    ...
    <div class="marginTop boxEdge1">
        ...
        <form:form action="${pageContext.request.contextPath}/front_notice/pagingCandidatePublicity" method="get" modelAttribute="searchModel" onsubmit="if($(this).find('.search_ipt').val()=='关键字查询'){$(this).find('.search_ipt').val('');}return true;">
...
<div class="ctr marginTop">
    ...
    <div class="ContentList_conT left">
        <c:forEach items="${page.rows}" var="item">
        	<div class="list_box">
	            <span class="titleG"><a href="#">[${util:dictionary('GSLXDM',item.gslx)}]</a><a href="${pageContext.request.contextPath}/front_notice/candidatePublicityDetail/${item.id}">${item}</a></span>
	            <span class="right">发布日期：<fmt:formatDate value="${item.gskssj}" pattern="yyyy-MM-dd"/></span>
	            <div class="clear"></div>
        ...
        <sys:page href="${pageContext.request.contextPath}/front_notice/pagingCandidatePublicity" page="${page}"/>
        <script>
