...
<div class="ctr content">
    ...
    <div class="marginTop boxEdge1">
        ...
        <form:form action="${pageContext.request.contextPath}/front_notice/pagingNotice" method="get" modelAttribute="searchModel" onsubmit="if($(this).find('.search_ipt').val()=='关键字查询'){$(this).find('.search_ipt').val('');}return true;">
...
<div class="ctr marginTop">
    ...
    <div class="ContentList_conT left">
        <c:forEach items="${page.rows}" var="item">
        	<div class="list_box">
	            <span class="titleG"><a href="#">[${util:dictionary('GGXZDM',item.ggxzdm)}]</a><a href="${pageContext.request.contextPath}/front_notice/noticeDetail/${item.id}">${item}</a></span>
	            <span class="right">发布日期：<fmt:formatDate value="${item.ggfbsj}" pattern="yyyy-MM-dd"/></span>
	            <div class="clear"></div>
        ...
        <sys:page href="${pageContext.request.contextPath}/front_notice/pagingNotice" page="${page}"/>
        <script>
