...
<div class="ctr content">
    ...
    <div class="address_nav">
        ...
        <div class="address_main left">
            当前位置：<a href="${util:property('frontEnd')}">首页</a> > <a href="#">项目信息</a>
        </div>
        <!-- 4.1.2通知 -->
    ...
    <div class="marginTop boxEdge1">
        ...
        <form:form action="${pageContext.request.contextPath}/front_projectInstance/pagingProjectInstance" method="get" modelAttribute="searchModel" onsubmit="if($(this).find('.search_ipt').val()=='关键字查询'){$(this).find('.search_ipt').val('');}return true;">
...
<div class="ctr marginTop">
    ...
    <div class="ContentList_conT left">
        ...
        <div class="list_box">
            <span class="ico_Xmxx"><a href="${pageContext.request.contextPath}/front_projectInstance/projectInstanceDetail/${item.id}">${item.zbxmmc}</a></span>
            <span class="right">发布日期：<fmt:formatDate value="${item.lxsj00}" pattern="yyyy-MM-dd"/></span>
            <span class="Xm_con">${item.nryfw0}</span>
            <div class="clear"></div>
        </div>
        </c:forEach>
        <sys:page href="${pageContext.request.contextPath}/front_projectInstance/pagingProjectInstance" page="${page}"/>
        <script>
