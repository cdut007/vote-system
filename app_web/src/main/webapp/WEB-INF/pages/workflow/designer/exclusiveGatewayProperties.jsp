...
<!--
	...
	function saveGatewayProperties() {
		task.gatewayId = jq('#id').val();
		task.gatewayName = jq('#name').val();
		task.documentation = jq('#documentation').val();
		jq.messager.alert('系统提示', '保存成功!', 'info');
	}
	function populateTaskProperites() {
		jq('#id').val(task.gatewayId);
		jq('#name').val(task.gatewayName);
		...
		<div id="task-properties-accordion" class="easyui-accordion" fit="true" border="false">
			<div id="general" title="基本配置" selected="true" class="properties-menu">
				<table id="general-properties">
					...
					<tr>
						<td align="right">默认方向:</td>
						<td>
							<input type="text" id="default" name="default" value="" />
						</td>
					</tr>
				</table>
			</div>
			<div id="main-config" title="主配置" class="properties-menu">
