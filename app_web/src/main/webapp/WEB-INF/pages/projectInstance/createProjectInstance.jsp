...
$(document).ready(function(){
	$("#zbfs_sel").change(function(){
		if($(this).children("option:selected").val() == 9){
			$.messager.alert('系统提示','目前只支持公开招标和邀请招标');
...
<table width="98%" style="margin:0 auto;">
	...
	<tr>
		<td class="need">*</td>
		<td>招标项目名称：</td>
		<td colspan="5"><input name="zbxmmc" value="${zbxmmc}" class="inputxt easyui-validatebox" datatype="*1-100" errormsg="请输入招标项目名称！" data-options="required:true,validType:'length[0,100]'" style="width: 672px" /></td>
		<td>
	...
	<tr>
		...
		<td>
			<input name="" disabled="disabled" value="" class="inputxt easyui-validatebox" placeholder="无需填写，自动生成" style="font-size:12px"/>
		</td>
		<td></td>
		<td class="need"></td>
		<td width="120px">机构内部招标编号：</td>
		<td>
			<input name="organProjectCode"  value="${organProjectCode}" class="inputxt easyui-validatebox" data-options="validType:'length[0,128]'" />
		</td>
	</tr>
	<tr>
		<td class="need">*</td>
		...
		<td class="need">*</td>
		<td>申报责任人：</td>
		<td><input name="zbzrr0" value="${zbzrr0}" class="inputxt easyui-validatebox" datatype="*1-50" errormsg="申报责任人！" data-options="required:true,validType:'length[0,25]'" /></td>
		<td>
			<div class="Validform_checktip"></div>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<select name="reviewType" class="easyui-validatebox" data-options="required:true" id="reviewType">
				...
				<option value="1" ${reviewType=="1"?"selected":""}>资格预审</option>
				<option value="2" ${reviewType=="2"?"selected":""}>资格后审</option>
			</select>
		</td>
		<td>
			<div class="Validform_checktip"></div>
		...
		<td>
			<select name="prereviewType" id="prereviewType">
				...
				<option value="2" >合格制</option>
				<option value="1" >合格制（打分）</option>
				<option value="3" >有限数量制</option>
			</select>
		</td>
		<td>
			<div class="Validform_checktip"></div>
	...
	<tr>
		<td class="need">*</td>
		<td>招标内容与范围:</td>
		<td colspan='5'><textarea name="nryfw0" data-options="required:true" style="width:672px;height:100px" class="inputxt easyui-validatebox" data-options="required:true,validType:'length[0,1000]'">${nryfw0}</textarea></td>
		<td>
			<div class="Validform_checktip"></div>
		</td>
	</tr>
	<tr>
		<td class="need">*</td>
		<td>审核部门名称：</td>
		<td> <input name="shbmmc" value="${shbmmc}" class="inputxt easyui-validatebox" data-options="required:true,validType:'length[0,100]'" /></td>
		<td>
			<div class="Validform_checktip"></div>
		</td>
	...
	<tr>
		<td class="need">*</td>
		<td>监督部门名称：</td>
		<td> <input name="jdbmmc" value="${jdbmmc}" class="inputxt easyui-validatebox" datatype="*1-100" errormsg="请输入监督部门名称！" data-options="required:true,validType:'length[0,100]'" /></td>
		<td>
			<div class="Validform_checktip"></div>
		</td>
		<td class="need"></td>
	...
	<tr>
		<td class="need">*</td>
		<td>监督部门负责人：</td>
		<td> <input name="jdbmfzr" value="${jdbmfzr}" class="inputxt easyui-validatebox" data-options="required:true,validType:'length[0,25]'"/></td>
		<td>
			<div class="Validform_checktip"></div>
		</td>
		<td class="need">*</td>
		<td>监督部门电话：</td>
		<td><input name="jdbmdh" value="${jdbmdh}" class="inputxt easyui-validatebox"  errormsg="请输入监督部门电话！"  datatype="s1-100"  data-options="required:true,missingMessage:'请输入监督部门电话！',validType:'MobileOrPhone'" /></td>
		<td>
			<div class="Validform_checktip"></div>
		</td>
	</tr>
