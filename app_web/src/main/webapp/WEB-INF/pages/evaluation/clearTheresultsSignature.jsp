...
%>
<c:set value="${tenderItemService.clearTheResultsPdf(projectItemId,tenderItemId)}" var="tenderItem" />
<c:set value="${userService.retrieveExpertSign(processBusinessKey)}" var="expertList" />
<c:set value="${expertApplyService.getExpertSealSignList(processBusinessKey,tenderItem.id,'CLEAR_RESUILTS')}" var="expertSignList" />
...
<script type="text/javascript">
	...
	function HWPostil1_NotifyCtrlReady() {
	    ...
	    OpenFile("${pageScope.basePath}/attachment/download/${tenderItem.clearTheresultsPdf}");
	}
	...
	$(function() {
		...
		$("#agree").click(function() {
			...
			var paramsArray = {"tenderItemId":"${tenderItem.id}","expertApplyId":"${processBusinessKey}","reviewType":"CLEAR_RESUILTS","projectItemId":"${projectItemId}"};
            var returnValue = SaveDocArray(paramsArray,"${pageScope.basePath}/evaluation/theResultsShallBeSigned");
            if ("OK" == returnValue) {
                $("#workflowForm").submit();
                ShowMessage("操作成功！");
            } else {
            	ShowMessage("操作成功！");
                window.location.href="/evaluation/clearTheresultsSignature?taskId=${taskId}&projectItemId=${projectItemId}&expertApplyId=${processBusinessKey}&tenderItemId="+returnValue+"&currentStage=CLEAR_RESUILTS";
            }
		});
...
<body>
	...
	<div class="block">
		<div id="ur_here">
			...
			<code>&gt;</code>
			清标结果签章确认(${tenderItem.organBName})
		</div>
		...
		<div class="usBox">
			...
			<div id="disagreeDiv" >
				...
				<%if(signStatus==0){%>
				 	...
				 	<input type="button" class="button red" value="确认结果" id="agree" />
				<%}%>
