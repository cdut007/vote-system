...
<head>
<title>${reviewStage=='1'?'详细评审统计表':'初步评审统计表'}</title>
<sys:link easyui="true"/>
<script type="text/javascript">
$(function(){
	$(".exportXLS").each(function(i){
		$(this).click(function(){
			var form=$("<form/>").attr("action",ctx+"/attachment/expertXLS").attr("method","POST").css("display","none").appendTo($("body"));
			form.append($("<input/>").attr("name","html").val($(".exportHTML").eq(i).html()));
			form.append($("<input/>").attr("name","exportName").val("${statistics.projectItem.projectInstance}初步评审表.xls"));
			form.submit();
		});	
	});
});
