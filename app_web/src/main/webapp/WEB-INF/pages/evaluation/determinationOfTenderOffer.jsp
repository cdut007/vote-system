...
<c:set value="${expertApplyService.getExpertSealSignList(processBusinessKey,projectItemId,'DECIDE_PRICE')}" var="expertSignList" />
<%
...
<script type="text/javascript">
    var sealNum=0;
	$(function() {
		console.log("${processBusinessKey}");
		...
		$("#submitBtn").click(function(){
			...
			if(r==1){
                ...
                var paramsArray = {"taskId":"${taskId}","projectItemId":"${projectItemId}","expertApplyId":"${processBusinessKey}","agree":"true","currentStage":"DECIDE_PRICE"};
                var returnValue = SaveDocArray(paramsArray,"${pageScope.basePath}/evaluation/completeFormDeterminationOfTenderOffer");
                if ("NO" == returnValue) {
                    ShowMessage("文件上传成功");
                    window.location.href="/evaluation/benchmarkTableSignature?taskId=${taskId}&projectItemId=${projectItemId}&expertApplyId=${processBusinessKey}&agree=true&currentStage=DECIDE_PRICE";
                } else {
                	if("workflow/taskNotFound"==returnValue){
                    	ShowMessage("任务不存在或已被处理");
                		window.location.href="/home";
                	}
                	else if ("OK" == returnValue) {
                        ShowMessage("文件上传成功");
                        window.location.href="/home";
                    }else{
                		ShowMessage("文件上传失败");
	...
	function HWPostil1_NotifyCtrlReady() {
        ...
        OpenFile("${pageScope.basePath}/attachment/download/${projectItem.attachmentPdfId}");
    }
...
<body>
	...
	<div class="block">
		...
		if(expertList.size()<=expertSignList.size()||hasSign(expertSignList,currUser.getId())){
			...
			if(signStatus==-1) out.print("&nbsp;&nbsp;&nbsp;&nbsp;（请等待前面的专家签章完！）");
		}
		%>
	</div>
	<div class="block">
