...
<script type="text/javascript">
    ...
    function HWPostil1_NotifyCtrlReady() {
        ...
        console.log("${pageScope.basePath}/attachment/download/${recruitFile.pdfAttachment.fjid00}");
		...
		<div id="score_dialog">
			...
			<form id="score_form" action="${pageContext.request.contextPath}/evaluation/saveScore" method="post">
				...
				<div class="block" style="width:100%;">
					<c:forEach items="${scoreReviewType}" var="scoreReviewType" varStatus="status">
						<c:set var="scoreReviewList" value="${scoreReviewType.scoreReviewList}" />
						<h2 class="tableTitle">${status.count}、${scoreReviewType.groupName}(共计：${scoreReviewType.fullScore})</h2>
						<c:choose>
							<c:when test="${scoreReviewType.groupFlag=='price'}">
								<table class="mytableStyle">
									...
									<tr>
										<td>${scoreReviewType.groupName}</td>
										<td style="text-align:left;text-indent: 2em" id="pbbfss">${scoreReviewList[0].reviewStandard}</td>
										<td align="right">
											<input type="text" readonly="readonly" class="easyui-numberbox" value="${priceScoreValue}" data-options="precision:2"/>
							...
							<c:otherwise>
								<table class='mytableStyle' id="scoreTable">
									...
									<tbody>
										<c:forEach items="${groupMap[scoreReviewType.groupName]}" var="groupItem" varStatus="status">
											<c:forEach items="${groupScoreMap[groupItem.groupName]}" var="item" varStatus="status">
											<tr>
												<th>${status.count}</th>
												<c:if test="${status.index==0}">
													<td rowspan="${fn:length(groupScoreMap[groupItem.groupName])}">${item.groupName}</td>
												</c:if>
