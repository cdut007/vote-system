...
<c:set value="${evaluationReportService.retrieveCandidate(expertApplyId,projectItemId)}" var="candidateList" />
		...
		<p>
			<span>按照招标文件规定的评标原则和评标办法，各评委独立进行了打分，汇总打分结果，投标人排序如下：</span>
		</p>
		...
		<table class="mytableStyle" align="center" style="width:100%;background-color:#fff;">
			...
			<tbody>
				<c:forEach items="${bidderSortList}" var="bidderSort" varStatus="s">
					<tr>
						<td>${s.count}</td>
						<td>${bidderSort.organName}</td>
						<c:if test="${(uniqueMethodId==null)}">
							<td><fmt:formatNumber value="${bidderSort.scoreValue}" maxFractionDigits="2" minFractionDigits="2" /></td>
						</c:if>
						<c:if test="${(uniqueMethodId=='JT_ZHPF_METHOD' || uniqueMethodId=='JT_REASONABLE_PRICE_METHOD')}">
							<td><fmt:formatNumber value="${bidderSort.scoreValue}" maxFractionDigits="3" minFractionDigits="3" /></td>
						</c:if>
					</tr>
				</c:forEach>
			</tbody>
		...
		<c:forEach items="${candidateList}" var="candidate" varStatus="s">
			<p>
				<span>第${s.count}名：</span><span style="text-decoration:underline">${candidate.organName}</span>
			</p>
		</c:forEach>
