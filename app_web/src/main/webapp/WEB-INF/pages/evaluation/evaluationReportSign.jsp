...
<c:set value="${expertApplyService.getExpertSealSignList(processBusinessKey,projectItemId,'REPORT')}" var="expertSignList" />
<%
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#submitBtn").click(function(){
			...
			//	return;
			//}
			//$.messager.confirm("系统提示", "您确定要提交吗？", function(y) {
			//	if(y){
					//*****如果印章位置有更新，那么更新印章位置*****************************
			//		sealUtil.checkAndUpdateSealPos(sealName,"${evaluationReport.id}",posX,posY);
					//******************************************
			//		$("#evaluationReportSignForm").submit();
			//	}
			//});
		...
		//	if(strObjectName != ""){//盖第二个章
		//		posX = 370;
		//		if(DWebSignSeal.FindSeal(strObjectName,0)!=""){//盖第三个章
		//			posX = 540;
		//		}
		//	}
		//	sealUtil.addSealWithBKey("${evaluationReport.id}",sealName,"data=string2sign","sealPos",posX, posY);
		//	$(document).scrollTop($("#juryAddSeal").offset().top);
		//});
	});
