...
var _27d=$("<a class=\"button\" href=\"javascript:void(0)\"></a>").appendTo(_27c);
if(p.handler){
_27d[0].onclick=p.handler;
}
//TODO 本地化按钮样式
_27d.css("margin-left","5px").addClass(p.color?p.color:"red").html(p.text);
//_27d.linkbutton(p);
}
}else{
...
$("<a></a>").attr("href","javascript:void(0)").text(_294).css("margin-left",10).bind("click",eval(_293[_294])).addClass("button red").appendTo(tb);
}
}
...
//TODO
if($._data(_3e1).events&&$._data(_3e1).events["click"]&&$._data(_3e1).events["click"].length>1){
	var handler=$._data(_3e1).events["click"][1].handler;
	$(_3e1).off("click",handler);
	_3e1.onclick=handler;
}
//TODO
...
if(opts.pagination){
$.extend(_6ae,{page:opts.pageNumber,pageSize:opts.pageSize});
}
if(opts.sortName){
...
$(_71d).html(_720.join(""));
},getValue:function(_725,_727){
	//TODO 本地化根据列名 获取json值
	if (_725 == null || _725 == undefined) {
		return null;
	}
	var filedArray = _727.split(".");
	var val = _725;
	for (var i = 0; i < filedArray.length; i++) {
		var key = filedArray[i];
		if (Object.prototype.toString.call(val) === '[object Array]') {
			val = $(val).map(function(){return this[key];}).get();
		} else if(val!=null&&val!=undefined){
			val = val[key];
		}
	}
	if(val != null && val != undefined&&Object.prototype.toString.call(val) === '[object Array]'){
		val=$(val).map(function(){
			if(this&&this["_toString"]){
				return this["_toString"];
			}
			return this;
		}).get().join(",");
	}else if(val&&val["_toString"]){
		val=val["_toString"];
	}
	return val;
...
//TODO 本地化根据列名获取json值
//var _728=_725[_727];
var _728=this.getValue(_725,_727);
...
if(opts.pagination){
$.extend(_7f4,{page:opts.pageNumber,pageSize:opts.pageSize});
}
if(opts.sortName){
